...
export default function AdminProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    if (!products) return [];
    const sorted = [...products].sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
      ...
      <div className="min-h-screen flex flex-col bg-black text-white font-arabic">
        ...
        <main className="flex-grow container mx-auto px-4 py-12">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <div>
              <h1 className="text-4xl font-black gold-text">المنتجات</h1>
              <p className="text-white/40 text-sm mt-1">إدارة جميع القطع المتاحة في NOVA</p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="relative w-full md:w-80 group">
                ...
                <Input placeholder="ابحثي عن منتج..." className="h-12 pr-12 bg-white/5 border-white/10 rounded-2xl text-white font-bold" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
            ...
            <Table>
              <TableHeader className="bg-white/5">
                <TableRow className="border-white/5">
                  ...
                  <TableHead className="text-white/60 font-black text-right text-center">الإجراءات</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredProducts.map((product: any) => (
                  <TableRow key={product.id} className="border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <TableCell><Badge variant="outline" className="border-white/10 text-white/60 text-[10px]">{product.categoryName}</Badge></TableCell>
                    <TableCell className="font-black text-primary">{product.price?.toLocaleString()} د.ع</TableCell>
                    <TableCell>
                      <div className="flex items-center justify-center gap-3">
                        <button onClick={() => toggleStatus(product.id, product.status)} className="p-2 bg-white/5 rounded-lg text-white/40 hover:text-white transition-all">
                          {product.status === 'active' ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                        </button>
                        <button onClick={() => handleDelete(product.id)} className="p-2 bg-red-500/10 rounded-lg text-red-500 hover:bg-red-500 hover:text-white transition-all"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
